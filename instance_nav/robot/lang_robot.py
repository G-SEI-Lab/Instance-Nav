...
class LangRobot:
    ...
    def get_scene_data_dirs(self):
        return self.scene_data_dirs
    ...
    def load_code(self, code_dir: str, task_i: int):
        ...
        code = "".join(code)
        return code
    # HabitatLanguageRobot implements _set_nav_curr_pose.
    ...
    def move_to_color_object(self, name: str, color_list: List[List[int]], true_color_value: List[List[int]], vis: bool = False):
        self._set_nav_curr_pose()
        pos = self.map.get_nearest_and_similarity_color_pos(self.curr_pos_on_map, name, color_list, 0.95, 5, vis)
        self.move_to_v2(pos)

    def get_object_pos(self, name: str, color_list: List[List[int]], vis: bool = False):
        pos = self.map.get_nearest_and_similarity_color_pos(self.curr_pos_on_map, name, color_list, 1.0, 50, vis)
        return pos

    def move_to_color_3d_object(self, name: str, color_list: List[List[int]], true_color_value: List[List[int]], floor: int, weight: float=0.875, vis: bool = False):
        self._set_nav_curr_pose()
        # color_list = [[56,98,173]]
        pos, obj_height_avg = self.map.get_3d_nearest_and_similarity_color_pos(self.curr_pos_on_map, name, color_list, floor, weight, vis)
