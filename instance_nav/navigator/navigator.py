...
from instance_nav.utils.navigation_utils import build_visgraph_with_obs_map, plan_to_pos_v2, plan_to_pos_v3, plan_to_pos_v3_bidirectional
...
class Navigator:
    ...
    def plan_to_v2(
        self, passable_map: np.ndarray, start_full_map: Tuple[float, float], goal_full_map: Tuple[float, float], vis: bool = False, floor: int = 0
    ) -> List[List[float]]:
        """
        Take full map start (row, col) and full map goal (row, col) as input
        Return a list of full map path points (row, col) as the palnned path
        ...
        start = self._convert_full_map_pos_to_cropped_map_pos(start_full_map)
        goal = self._convert_full_map_pos_to_cropped_map_pos(goal_full_map)
        paths = plan_to_pos_v3_bidirectional(start, goal, passable_map, vis, floor)
        paths = self.shift_path(paths, self.rowmin, self.colmin)
        return paths
