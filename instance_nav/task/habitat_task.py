...
from instance_nav.utils.habitat_utils import agent_state2tf
...
class HabitatTask:
    ...
    def setup_scene(self, scene_loader: HabitatSceneLoader):
    ...
    def test_actions(
        ...
        for action_i, action in enumerate(actions_list):
            if action == "stop":
                ...
                row, col, angle_deg = self.scene_loader.convert_habitat_tf_to_full_map_pose(hab_tf)
    ...
    def _check_min_dist_to_goal_positions(
        ...
        return min_dist_pix * self.scene_loader.cs
