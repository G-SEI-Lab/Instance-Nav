...
from instance_nav.utils.habitat_utils import get_position_floor_objects
...
class HabitatObjectNavigationTaskAcross(HabitatTask):
    def load_task(self):
        ...
        task_path = Path(self.scene_loader.data_dir) / "color_object_3d_nav_dataset.json"
    ...
    def test_step_v2(self, sim: habitat_sim.Simulator, robot: HabitatLanguageRobot, action: str, agent_position: np.array = None, vis: bool = False):
        ...
        if action == "stop":
            ...
            if target_object is None:
                ...
                return
            closest_object, closest_dist, closest_height_dist = self.search_calculate_distance_object_by_id(robot, target_object["object_id"], agent_position, vis)
            ...
            if -0.5 < closest_height_dist < 2.5:
                self.reach_same_floor.append(self.curr_subgoal_id)
                if closest_dist < 1.05:
                    self.finished_subgoals.append(self.curr_subgoal_id)
                    logging.info(f"###({self.curr_subgoal_id + 1}/{self.n_subgoals_in_task}) {next_subgoal_name} reached! Distance: {closest_dist:.2f}m,Height difference: {closest_height_dist:.2f}m###")
                else:
                    logging.info(f"###({self.curr_subgoal_id + 1}/{self.n_subgoals_in_task}) {next_subgoal_name} unreached! Distance: {closest_dist:.2f}m,Height difference: {closest_height_dist:.2f}m###")
            else:
                logging.info(f"###({self.curr_subgoal_id + 1}/{self.n_subgoals_in_task}) {next_subgoal_name} unreached! Not on the same floor, Height difference: {closest_height_dist:.2f}m###")
            if vis:
    ...
    def save_single_task_metric_v2(
        ...
        results_dict["reach_same_floor"] = self.reach_same_floor
    ...
    def search_calculate_distance_object_by_id(self, robot: HabitatLanguageRobot, object_id: str, pos_hab: np.ndarray, vis: bool = False):
        ...
        obj_pos = closest_obj.aabb.center  # 3D 世界坐标
        closest_height_dist = obj_pos[1] - pos_hab[1]
        map = robot.map
        # 转换为 2D 网格坐标
        row, col = self.world_to_grid(robot, obj_pos)
        logging.info(f"Object 3D position: [{obj_pos[0]:.3f}, {obj_pos[1]:.3f}, {obj_pos[2]:.3f}], Grid (row, col): ({row}, {col})")
        ...
        return closest_obj, closest_dist, closest_height_dist
